...
function initializeSignalR() {
    ...
    if (typeof requestType === "string" && requestType.toLowerCase().includes("civil")) {
    ...
    signalRConnection.on("ReceiveNewRequest", (newRequest) => {
        ...
        if (newRequest.requestType.toLowerCase().includes("civil")) {
...
async function loadRequestsTable(pageNumber = currentRequestsPage, pageSize = requestsPageSize, status = null, type = null, searchTerm = null) {
    ...
    let queryParams = `?pageNumber=${pageNumber}&pageSize=${pageSize}&type=CivilDocument`;
    ...
    try {
        ...
        console.log("API: Civil Requests Loaded: ", pagedResult);
        ...
        if (pagedResult.items?.length > 0) {
            pagedResult.items.forEach(req => {
                ...
                // حماية من null في status
                const isNewRequest = (req.status || '').toLowerCase() === "new";

                const actionButtons = isNewRequest
                    ? `<button class="btn btn-sm btn-success me-1" onclick="approveRequest('civil', '${req.requestId}')">
                          <i class="fas fa-check"></i> Approve
                       </button>
                       <button class="btn btn-sm btn-danger me-1" onclick="rejectRequest('civil', '${req.requestId}')">
                ...
                dataForTable.data.push([
                    ...
                    formattedDate,
                    req.status || 'N/A',
                    `<button class="btn btn-sm btn-info me-1" onclick="viewRequestDetails('civil', '${req.requestId}')">
        ...
        datatableInstance = new simpleDatatables.DataTable(tableElement, {
            ...
            labels: {
                ...
                noRows: "No civil document requests found.",
                info: "Showing {start} to {end} of {rows} entries"
            }
        });

    } catch (error) {
        showToastNotification("Error loading civil document requests.", "error");
        if (datatableInstance) {
            datatableInstance.destroy();
            datatableInstance = new simpleDatatables.DataTable(tableElement, {
...
// استبدل دالة approveRequest بهذا:
async function approveRequest(requestType, requestId) {
    ...
    try {
        ...
        showToastNotification(`Civil document request approved successfully.`, "success");
...
// استبدل دالة rejectRequest بهذا:
async function rejectRequest(requestType, requestId) {
    ...
    try {
        const result = await fetchData(`${API_BASE_URL}/requests/${requestType}/${requestId}/reject`, {
            method: "POST",
            body: JSON.stringify({ status: "Rejected", notes: "All documents verified. Rejected" })
        });
        
        showToastNotification(`Civil document request rejected successfully.`, "success");
...
async function viewRequestDetails(requestType, requestId) {
    try {
        ...
        console.log("Civil Request Details:", details);
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="requestDetailsModalLabel">Civil Document Request: ${details.requestId || 'N/A'}</h5>
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("refreshRequests")?.addEventListener("click", () => {
        showToastNotification("Refreshing civil document requests...", "info");
        loadRequestsTable();
    });
